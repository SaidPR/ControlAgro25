...
export default class UsersViewModel {
  ...
  fetchUsers = async () => {
    ...
    try {
      ...
      const q = query(workersRef, where("role", "==", "TRABAJADOR")); // Filtrar por rol "TRABAJADOR"
      const querySnapshot = await getDocs(q);
      const usersList = querySnapshot.docs.map((doc) =>
        new UserModel({ id: doc.id, ...doc.data() })
      );

      if (usersList.length === 0) {
        Alert.alert("Sin resultados", "No se encontraron trabajadores.");
      }
